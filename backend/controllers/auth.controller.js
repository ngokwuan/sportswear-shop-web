...
export const login = async (req, res) => {
  try {
    ...
    // Cookie config đơn giản cho same domain
    res.cookie('jwt', token, {
      httpOnly: true,
      maxAge: maxAge,
      secure: process.env.NODE_ENV === 'production', // HTTPS trong production
      sameSite: process.env.NODE_ENV === 'production' ? 'strict' : 'lax',
      path: '/', // Đảm bảo cookie available cho toàn bộ site
    });

    console.log(
      `Login successful for user: ${email}, Environment: ${process.env.NODE_ENV}`
    );

    const responseData = {
      message: 'Đăng nhập thành công!',
      rememberMe: rememberMe || false,
      // Không trả về accessToken nữa vì dùng cookie
      id: existUser.id,
      email: existUser.email,
      name: existUser.name,
      role: role,
