...
function Checkout() {
  ...
  const handleVnPayment = async () => {
    ...
    if (!validateForm()) {
      showNotification('error', 'Vui lòng kiểm tra và điền đầy đủ thông tin');
      return;
    }

    const total = calculateTotal();
    if (total <= 0) {
      showNotification('error', 'Giỏ hàng không có sản phẩm hợp lệ');
      return;
    }
    ...
    try {
      ...
      const userData = JSON.parse(user);
      if (!userData?.id) {
        showNotification('error', 'Thông tin người dùng không hợp lệ');
        navigate('/login');
        return;
      }
      ...
      const paymentData = {
        amount: Math.round(total), // VNPay yêu cầu số nguyên
        orderInfo: `Thanh toán đơn hàng #${orderId} - ${orderInfo.fullName}`,
        language: 'vn',
        bankCode: '', // Để trống để user chọn phương thức thanh toán
        // Thêm thông tin đơn hàng vào metadata nếu cần
        metadata: {
          order_id: orderId,
          user_id: userData.id,
      ...
      if (response.data.success) {
        ...
        const errorMessage =
          response.data.message || 'Tạo đơn thanh toán thất bại';
      ...
      if (error.response) {
        ...
        if (status === 400) {
          ...
          errorMessage =
            data.message || 'Không tìm thấy sản phẩm trong giỏ hàng';
        } else if (status === 500) {
